...
function typewriterEffect(elementId, text, speed = 50) {
    ...
    const type = () => {
        if (index < lines.length) {
            const line = lines[index].trim(); // Remove leading/trailing spaces
            element.innerHTML += line + '<br>'; // Add the line with a line break
            index++;
            setTimeout(type, speed * line.length); // Adjust delay based on line length
        }
    };
